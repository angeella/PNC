...
$$\Pr( z_{\alpha/2} \le (\bar{X} - \mu)/(\sigma/\sqrt{n}) \le z_{1-\alpha/2}) = 1-\alpha$$
...
$$
IC_{1-\alpha}(\mu) = (\mu + z_{\alpha/2} \,\, \sigma / \sqrt{n}, \mu + z_{1-\alpha/2} \,\, \sigma / \sqrt{n} )
$$
...
$$
IC_{1-\alpha}(\pi) = [\hat{\pi} + z_{\alpha/2} \,\, \sqrt{(\hat{\pi}(1-\hat{\pi}))/n}, \pi + z_{1-\alpha/2} \,\, \sqrt{(\hat{\pi}(1-\hat{\pi}))/n} )]
$$
...
$$
IC_{1-\alpha}(\mu_1 - \mu_2) = (\bar{X}_1 - \bar{X}_2 + z_{\alpha/2} \,\, \sqrt{\sigma^2_{\bar{X}_1 - \bar{X}_2}}, \bar{X}_1 - \bar{X}_2 + z_{1-\alpha/2} \,\, \sqrt{\sigma^2_{\bar{X}_1 - \bar{X}_2}} )
$$
...
The parameter of interest in this case becomes the difference of means $\mu_d = \mu_1 - \mu_2$. We then consider the estimator $\bar{D} = \sum_{i=1}^{n} \dfrac{X_{1i} - X_{2i}}{n}$. Since also here we do not know $\sigma^2_{D} = \sigma^2_{\mu_1 - \mu_2}$ we plug-in the estimator

$$s_D^2 = \dfrac{1}{n-1} \sum_{i}^{n} (D_i - \bar{D})^2.$$
Finally the confidence intervals at level $1-\alpha$ equals:

$$
IC_{1-\alpha}(\mu_d) = [\bar{D} + t_{\alpha/2,n-1} \,\, \sigma_{d}\sqrt{n}, \bar{D} + t_{1- \alpha/2, } \,\, \sigma_{d}/\sqrt{n} ]
$$
...
you must specify if the observations are paired or not in the `paired` argument, and if we must assume that the two variances are equals in the `var.equal` argument. 
...
First of all, let's compute the difference of AHI total scores between the first and last occurrences for each type of interventions.
